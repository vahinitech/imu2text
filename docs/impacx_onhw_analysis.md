...
significance filtering and NCA dimensionality reduction). It is a counterpoint
to neural pipelines like ours: no training beyond distance computation and
easy to interpret, but expensive at inference (DTW-kNN compares every test
sample against every training sample).
...
Practices worth adopting (as ideas, not code): caching the distance matrix,
evaluating all 5 official folds, and reporting WD and WI separately. Weaknesses: `O(N_train ·
N_test)` DTW cost, and simple min–max per-sample scaling (our pipeline's
standardization is fitted on the training set only, so it does not leak).
...
1. **`set_size(width, fraction)`** converts a LaTeX document width **in
   ...
   the golden ratio ((√5−1)/2). This is a common approach for publication
   figures, described at
   [jwalton.info](https://jwalton.info/Embed-Publication-Matplotlib-Latex/):
   the figure is created at exactly `\textwidth`, so LaTeX never rescales it
   and fonts stay at their true size.
...
The mean±std "cloud" (`fill_between`) applies here once
`scripts/make_learning_curve.py` is run with multiple seeds; it is the same
plotting hook ImpAcX left commented out.
