...
def plot_learning_curve(width: float) -> None:
    if not os.path.exists(CURVE_CSV):
        print(f"skip learning-curve figure ({CURVE_CSV} not found; "
    ...
    try:
        (L, a, w0), _ = curve_fit(logistic, writers, acc,
                                  p0=[80.0, 0.15, 5.0], maxfev=20000)
        w_grid = np.linspace(1, FULL_SCALE_WRITERS, 200)
        ax.plot(w_grid, logistic(w_grid, L, a, w0), "--", color=GRAY,
                linewidth=1.2, label="logistic fit")
        proj = logistic(FULL_SCALE_WRITERS, L, a, w0)
        ax.plot([FULL_SCALE_WRITERS], [proj], "o", color=RED, markersize=5)
        ax.annotate(f"projected {proj:.1f}%\n@ {FULL_SCALE_WRITERS} writers",
                    (FULL_SCALE_WRITERS, proj), textcoords="offset points",
                    xytext=(-8, -24), ha="right", fontsize=8, color=TEXT)
        ax.axhline(L, color=MUTED, linewidth=0.6, linestyle=":")
        ax.annotate(f"ceiling L = {L:.1f}%", (1, L), textcoords="offset points",
                    xytext=(2, 4), fontsize=8, color=MUTED)
    except RuntimeError:
        print("logistic fit did not converge; plotting measurements only")

    ax.plot(writers, acc, "-o", color=BLUE, linewidth=2, markersize=5,
            label="measured (CNN+BiLSTM)")
    for wx, ay in zip(writers, acc):
        ax.annotate(f"{ay:.1f}", (wx, ay), textcoords="offset points",
                    xytext=(0, 6), ha="center", fontsize=7.5, color=TEXT)
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(RESULTS_DIR, f"learning_curve.{ext}"),
                    format=ext, bbox_inches="tight", dpi=200)
    plt.close(fig)
...
def plot_benchmarks(width: float) -> None:
    ...
    for yi, a in zip(y, accs):
        ax.annotate(f"{a:.1f}%", (a, yi), textcoords="offset points",
                    xytext=(4, 0), va="center", fontsize=8, color=TEXT)
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(RESULTS_DIR, f"model_benchmarks.{ext}"),
                    format=ext, bbox_inches="tight", dpi=200)
    plt.close(fig)
