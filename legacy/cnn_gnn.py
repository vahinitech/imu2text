...
from keras.layers import Conv1D, MaxPooling1D, Flatten, Dense, Input, Reshape, Dropout
...
log_message(
    f"Ground truth labels loaded successfully. Number of labels: {len(gt_data)}"
)
...
log_message(
    f"Ground truth labels converted successfully. Shape: {gt_data_categorical.shape}"
)
log_message(
    f"Step 1: Data Conversion completed in {time.time() - start_time:.2f} seconds"
)
...
imu_data_padded = pad_sequences(imu_data_normalized, padding="post", dtype="float32")
log_message(
    f"IMU data padded successfully. Data shape after padding: "
    f"{imu_data_padded.shape}"
)
log_message(
    f"Step 1: Data Preprocessing completed in {time.time() - start_time:.2f} seconds"
)
...
if os.path.exists(model_file):
    ...
    log_message(
        f"Step 2: Feature Extraction completed in {time.time() - start_time:.2f} seconds"
    )
    ...
    edge_index = (
        torch.tensor(
            [[i, i + 1] for i in range(imu_data_padded.shape[1] - 1)], dtype=torch.long
        )
        .t()
        .contiguous()
    )
    ...
    class GCN(torch.nn.Module):
        """Two-layer graph convolution over the IMU channel graph."""
        ...
        def forward(self, data):
            """Embed one graph batch into 128 dimensions."""
    ...
    log_message(
        f"Step 3: GNN Relationship Modeling completed in {time.time() - start_time:.2f} seconds"
    )
    ...
    regression_output = Dense(imu_data_padded.shape[1] * 13, activation="linear")(x)
    # Reshape regression output to match original IMU data dimensions
    regression_output = Reshape((imu_data_padded.shape[1], 13), name="regression")(
        regression_output
    )
    log_message(
        f"Regression head output shape after reshape: {regression_output.shape}"
    )
    ...
    mtl_model.compile(
        optimizer="adam", loss=["categorical_crossentropy", "mse"], metrics=["accuracy"]
    )
    ...
    mtl_model.fit(
        imu_data_padded,
        [gt_data_categorical, imu_data_padded],
        epochs=NUM_EPOCHS,
        batch_size=32,
        verbose=DISPLAY_TRAINING_PROGRESS,
    )
    log_message("MTL model training completed.")
    log_message(
        f"Step 4: MTL Model Training completed in {time.time() - start_time:.2f} seconds"
    )
...
classification_accuracy = float(
    np.mean([p == g for p, g in zip(predicted_chars, ground_truth_chars)])
)

# Create a table with IMU character, ground truth character, and accuracy
table_data = []
mismatched_data = []
for i, predicted_char in enumerate(predicted_chars):
    if predicted_char != ground_truth_chars[i]:
        mismatched_data.append([i + 1, predicted_char, ground_truth_chars[i]])
    if DISPLAY_ALL_RESULTS or predicted_chars[i] != ground_truth_chars[i]:
        table_data.append([i + 1, predicted_chars[i], ground_truth_chars[i]])
