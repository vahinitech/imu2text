---
name: benchmarking
description: Use when running, reporting, or reviewing any accuracy/CER/WER experiment in this repo, including training runs, benchmark tables, README result updates, PR descriptions quoting a number, or adding a dataset loader. Use when asked to "improve accuracy", "benchmark", "compare configs", "why is accuracy low", or when about to write a number into a doc.
---

# Benchmarking

Every rule here comes from a mistake made in this codebase, most of them in
PR #8.
...
A gain smaller than that spread is not a result. This is the most common way
to fool yourself here, because the small dataset runs fast and invites
iteration.

## Ensembles

Ensemble members share one split: same `--split-seed` and data flags,
different `--seed`. Average their probabilities with equal weight. Never
re-estimate BatchNorm statistics or fit normalisation on test data.
...
- **Comparable:** Ott et al., ACM MM 2022, **Table 3**, right-handed writers,
  on the same six official OnHW-chars splits this repo trains on. Their
  CNN+BiLSTM row: lower 79.48 WI, upper 85.60 WI, **combined 68.06 WI**. The
  last one is the competitor for our 52-class number.
...
- **Stale:** the "~64%" from IMWUT 2020. It is the older figure for the same
  architecture, and quoting it instead of 68.06 overstates our margin.
...
The known result on 52-class OnHW-chars: 43.1% of the 72.5% model's errors
(38.4% for the weaker 20-epoch baseline) are a letter confused with its own
other case, all twelve top confusions are case pairs, and for
...
it was worth +0.9 (70.9 against 70.0), and the full configuration with label
smoothing and the LR schedule reached 72.5, 3.3 points over the baseline.
...
Formats guessed wrong once, and worth checking first: labels as strings, not
ints; fold directories named `0`, not `fold_0`; labels right-padded with the
blank index; split files named differently from their unsplit equivalents.
Real archives also contain degenerate records: three OnHW-chars samples have
zero timesteps.
